...
FunctionMerger::merge(Function *F1, Function *F2, std::string Name, const FunctionMergingOptions &Options) {
  ...
  */
  ...
  SmallVector<BasicBlock *, 8> F1Blocks;
  SmallVector<BasicBlock *, 8> F2Blocks;
  //for (BasicBlock &BB : *F1) F1Blocks.push_back(&BB);
  //for (BasicBlock &BB : *F2) F2Blocks.push_back(&BB);
  orderBlocks(*F1, F1Blocks);
  orderBlocks(*F2, F2Blocks);
  ...
  PtrToRefContainer<BasicBlock, SmallVector<BasicBlock *, 8> > F2RefBlocks(F2Blocks);
  */
  SALSSACodeGen CG(F1RefBlocks, F2RefBlocks);
  Gen(CG);
...
unsigned instToInt(Instruction *I) {
  ...
  case Instruction::Load: {
    ...
    lValue += LI->getAlign().value();//TODO                       // Alignment
  ...
  case Instruction::Store: {
    ...
    sValue += SI->getAlign().value();//TODO                       // Alignment
	...
	Value *NewV = nullptr;
	  ...
	  if (PHI->getIncomingBlock(UI.getOperandNo()) == I->getParent())
	    continue;
          ...
          NewV = Builder.CreateLoad(I->getType(), Addr);
        } else {
