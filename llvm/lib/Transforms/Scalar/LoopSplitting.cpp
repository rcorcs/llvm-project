...
struct LoopSplitting : public FunctionPass {
  ...
  public:
    ...
    Value *calculateIntersectionPoint(const SCEVAddRecExpr *first, const SCEVAddRecExpr *second, CmpInst *compare, IRBuilder<> *builder) {
        ...
        const auto b1 = this->expander->expandCodeFor(first->getOperand(0), first->getOperand(1)->getType(), compare);
        const auto a1 = this->expander->expandCodeFor(first->getOperand(1), first->getOperand(0)->getType(), compare);
        const auto b2 = this->expander->expandCodeFor(second->getOperand(0), second->getOperand(1)->getType(), compare);
        const auto a2 = this->expander->expandCodeFor(second->getOperand(1), second->getOperand(0)->getType(), compare);
    ...
    Value *calculateIntersectionPoint(const SCEVAddRecExpr *add, Value *constant, CmpInst *compare, IRBuilder<> &builder) {
        ...
        assert((predicate == CmpInst::ICMP_UGE || predicate == CmpInst::ICMP_ULT || predicate == CmpInst::ICMP_ULE ||
               ...
               "We only treat integer inequalities for now");
        const auto b = this->expander->expandCodeFor(add->getOperand(0), constant->getType(), compare->getParent()->getFirstNonPHI());
        const auto a = this->expander->expandCodeFor(add->getOperand(1), constant->getType(), builder.GetInsertBlock()->getFirstNonPHI());
    ...
    bool isAffineAndConstantComparison(ScalarEvolution &SE, CmpInst &comparison) {
        ...
        if (!(predicate == CmpInst::ICMP_UGE || predicate == CmpInst::ICMP_ULT || predicate == CmpInst::ICMP_ULE ||
              predicate == CmpInst::ICMP_SGT || predicate == CmpInst::ICMP_SGE || predicate == CmpInst::ICMP_SLT ||
              predicate == CmpInst::ICMP_SLE || predicate == CmpInst::ICMP_UGT))
            return false;

        if (!SE.isSCEVable(comparison.getOperand(0)->getType()) || !SE.isSCEVable(comparison.getOperand(1)->getType())) return false;
    ...
    Loop *getLoop(CmpInst *comparison, LoopInfo &LI, ScalarEvolution &SE) {
        ...
        if (isAffineAndConstantComparison(SE, *comparison)) {
            ...
            auto loop = affine->getLoop(); // const reference, find it in
                                           // hierarchy :(
    ...
    SplitInfo *splitInforForAddRecAndConstant(ScalarEvolution &SE, LoopInfo &LI, CmpInst *instruction, const SCEVAddRecExpr *affine,
                                              Value *constant, bool firstIsConstant, Instruction *constantInstruction = nullptr) {
        auto loop = getLoop(instruction, LI, SE);

        auto insertionPoint = loop->getLoopPreheader()->getTerminator();
        IRBuilder<> builder(insertionPoint);

        if (constantInstruction != nullptr) builder.Insert(constantInstruction);
        ...
        auto nextIterationValue = expander->expandCodeFor(nextIterationScev, constant->getType(), insertionPoint);

        Value *v = builder.CreateICmp(instruction->getPredicate(), firstIsConstant ? constant : iterationValue,
                                      firstIsConstant ? iterationValue : constant);
        Value *nextPoint = builder.CreateICmp(instruction->getPredicate(), firstIsConstant ? constant : nextIterationValue,
        ...
        auto valueAfterLoop =
            expander->expandCodeFor(loopScev->evaluateAtIteration(nextScevValue, SE), constant->getType(), insertionPoint);
        auto splitInfo = new SplitInfo(value, loopEndValue, v, valueAfterLoop, nextPoint, instruction, firstIsConstant ? 0 : 1);
        return splitInfo;
    }
    std::vector<SplitInfo> comparisonToSplitInfo(ScalarEvolution &SE, LoopInfo &LI, std::set<CmpInst *> &comparisons) {
        ...
        for (CmpInst *instruction : comparisons) {
            if (isAffineAndConstantComparison(SE, *instruction)) {
                ...
                auto splitInfo = splitInforForAddRecAndConstant(SE, LI, instruction, affine, constant->getValue(), firstIsConstant);
                ...
                if (firstAffine->getLoop() == secondAffine->getLoop()) {
                    ...
                    auto firstNextIterationValue = expander->expandCodeFor(firstNextIterationScev, value->getType(), insertionPoint);

                    auto secondNextIterationScev = secondAffine->evaluateAtIteration(nextScevValue, SE);

                    auto secondNextIterationValue = expander->expandCodeFor(secondNextIterationScev, value->getType(), insertionPoint);
                    ...
                    auto loopScev = cast<SCEVAddRecExpr>(SE.getSCEV(exitCmp->getOperand(0)));
                    auto loopEndValue =
                        expander->expandCodeFor(loopScev->evaluateAtIteration(scevValue, SE), value->getType(), insertionPoint);

                    auto valueAfterLoop =
                        expander->expandCodeFor(loopScev->evaluateAtIteration(nextScevValue, SE), value->getType(), insertionPoint);
                    ...
                    if (firstLoop->contains(secondLoop)) {
                        SplitInfo *splitInfo = splitInfo =
                            splitInforForAddRecAndConstant(SE, LI, instruction, secondAffine, instruction->getOperand(0), true);
                        splitInfoVector.push_back(*splitInfo);
                    } else if (secondLoop->contains(firstLoop)) {
                        SplitInfo *splitInfo =
                            splitInforForAddRecAndConstant(SE, LI, instruction, firstAffine, instruction->getOperand(1), false);
                        splitInfoVector.push_back(*splitInfo);
                    }
                }
            }
    ...
    Loop *cloneLoop(Function *F, Loop *L, LoopInfo *LI, DominatorTree &DT, const Twine &NameSuffix, ValueToValueMapTy &VMap) {
    ...
    void splitLoop(ScalarEvolution &SE, Function *F, DominatorTree &DT, Loop *loop, LoopInfo *LI, SplitInfo *info) {
        ...
        IRBuilder<> builder(clonedLoop->getHeader()->getFirstNonPHI());
    ...
    bool runOnFunction(Function &F) override {
        ...
        if(splittedLoops > 0) {
            errs() << "\n\n   -> Modified function: " << F.getName() << "\n\n";
            errs() << "\n   -> Modified loops: " << splittedLoops << "\n";
...
static RegisterPass<LoopSplitting> Y("loopsplit", "Hello World Pass (with getAnalysisUsage implemented)");

Pass *llvm::createLoopSplittingPass() {
  return new LoopSplitting;
}
