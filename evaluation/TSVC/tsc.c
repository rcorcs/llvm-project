...
//void __attribute__ ((noinline)) check(int name){
void check(int name){
...
//	no dependence - vectorizable
	...
	struct timespec start_t, end_t; double clock_dif_sec;
...
//			X[i] = (Y[i] * Z[i])+(U[i]*V[i]);
	...
	long ns = end_t.tv_nsec - start_t.tv_nsec;
	if (start_t.tv_nsec > end_t.tv_nsec) { --seconds; ns += 1000000000; }
	clock_dif_sec = (double)seconds + (double)ns/(double)1000000000;
	printf("S000\t %.5f \t\t", clock_dif_sec);;
	check(1);
	return 0;
}
...
//	sum reduction
	...
	if (start_t.tv_nsec > end_t.tv_nsec) { --seconds; ns += 1000000000; }
	clock_dif_sec = (double)seconds + (double)ns/(double)1000000000;
	printf("S31111\t %.5f \t\t", clock_dif_sec);;
	check(1);
	return 0;
}
...
//	if to max with index reductio 2 dimensions
	...
	if (start_t.tv_nsec > end_t.tv_nsec) { --seconds; ns += 1000000000; }
	clock_dif_sec = (double)seconds + (double)ns/(double)1000000000;
	printf("S13110\t %.5f \t\t", clock_dif_sec);;
	temp = max + xindex+1 + yindex+1;
	check(-1);
	return 0;
}
