...
entry:
  ...
  %1 = alloca float, i8 4, align 4
  %2 = load float, float* %A, align 4, !tbaa !2
  %3 = tail call float @llvm.fabs.f32(float %2)
  %cmp = fcmp ogt float %3, %max
  %max.addr.0 = select i1 %cmp, float %3, float %max
  %arrayidx2 = getelementptr inbounds float, float* %A, i64 1
  %4 = load float, float* %arrayidx2, align 4, !tbaa !2
  %5 = tail call float @llvm.fabs.f32(float %4)
  %cmp3 = fcmp ogt float %5, %max.addr.0
  %max.addr.1 = select i1 %cmp3, float %5, float %max.addr.0
  ...
  %cmp8 = fcmp ogt float %7, %max.addr.1
  %max.addr.2 = select i1 %cmp8, float %7, float %max.addr.1
  %arrayidx12 = getelementptr inbounds float, float* %A, i64 3
  %8 = load float, float* %arrayidx12, align 4, !tbaa !2
  %9 = tail call float @llvm.fabs.f32(float %8)
  %cmp13 = fcmp ogt float %9, %max.addr.2
  %max.addr.3 = select i1 %cmp13, float %9, float %max.addr.2
